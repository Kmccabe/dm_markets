...
def three_d_grid(grid_size):
    ...
    Returns a scaffolding for building a three-dimensional graph.

    NOTE: Implemented for 2D Square grid. Can do other shapes in the future. Pass shape, dims.
    """ 
...
def plottable_locs(locs, grid_df, investigated="Agents"):
    """
    Unwraps lengths of dictionary of location-agents dictionary onto melted grid vectors.

    Presently implemented for number of agents at location.
    
    Args:
        locs (dict): a Dictionary of (location tuple): Observation.
        grid_df (pd.DataFrame): A flattened representation of a 2D Grid.
        investigated (string, optional): The name of the observation to extract. (column name)

    returns:
        a DataFrame of trial-week-period-x-y-observation
    """
    grid_new = grid_df.copy()
    grid_new[investigated] = 0
    for loc in locs.keys():
...
def graph_plotted(plotted, 
                  title_val=None, hue_val="Agents", hue_norm_val=(0, 1), 
                  size_val="Agent", size_norm_val=(0, 1), 
                  legend=False, palette="hot_r",
                  integer_bar=True, subtitle=False):
    """Visualize the location occupation data on a 2D grid to show the locations and values of the investigated value"""
...
def to_len_str(number, str_len):
    """Helper to make numbers alphabetically compatible."""
    base_str = str(number)
    new_str = '0'*(str_len-len(base_str)) + base_str
    return new_str
...
def movie_plotted(collated_plotted, movie_name=None, graph_folder=None, title_val=None, hue_val="Agents", size_val="Agents", scale="relative", hue_max=None, 
                  size_max=None, week_max=None, period_max=None, include_init=True, fps=1, subtitle=True,
                  default_movie_stem = "loc_movie", default_graph_folder="loc_movie"):
    """
    collated_plotted: one trial (or an average of trials) plotted using 
    movie_name: The name to save the movie under (if not provided - uses default for the notebook + timestamp)
    graph_folder: The name of the folder to save the graphs in (if not provided - uses default for the notebook + new folder timestamp)
